...
console.log('✅ journal-generator.js script file loaded and executing!');
window.journalScriptLoaded = true;

// Global variables to store user data
let userEntriesByDay = {};
let userReviewsByWeek = {};
let foundationEntries = [];
let goalsData = [];
let userName = '';
let userJoinDate = '';
...
async function generateMasterJournal() {
    try {
        ...
        console.log('Journal content element exists:', !!document.getElementById('journal-content'));
        
        // Try to get user data from window object (set by Next.js page or iframe parent)
        if (typeof window !== 'undefined' && window.userEntriesByDay) {
            userEntriesByDay = window.userEntriesByDay;
            userReviewsByWeek = window.userReviewsByWeek || window.reviewsByWeek || {};
            foundationEntries = window.foundationEntries || window.userFoundationEntries || [];
            goalsData = window.goalsData || window.userGoals || [];
            userName = window.userName || window.user_name || '';
            userJoinDate = window.userJoinDate || window.user_join_date || '';
            console.log(`✅ Loaded from window: ${Object.keys(userEntriesByDay).length} entries, ${Object.keys(userReviewsByWeek).length} reviews, ${foundationEntries.length} foundation entries, ${goalsData.length} goals`);
            if (foundationEntries.length > 0) {
                console.log('📊 Foundation entry fields:', Object.keys(foundationEntries[0]));
                console.log('📊 Foundation sample data:', {
                    what_drives_me: foundationEntries[0].what_drives_me?.substring(0, 50),
                    what_im_done_with: foundationEntries[0].what_im_done_with?.substring(0, 50),
                    who_im_building_for: foundationEntries[0].who_im_building_for?.substring(0, 50),
                    books_that_shaped_me: foundationEntries[0].books_that_shaped_me?.substring(0, 50),
                    mentors_role_models: foundationEntries[0].mentors_role_models?.substring(0, 50),
                    core_principles: foundationEntries[0].core_principles?.substring(0, 50),
                    my_values: foundationEntries[0].my_values?.substring(0, 50),
                    accountability_partner: foundationEntries[0].accountability_partner
                });
            }
        } else {
            ...
            try {
                ...
                if (token) {
                    ...
                    if (response.ok) {
                        ...
                        userReviewsByWeek = data.reviewsByWeek || {};
                        foundationEntries = data.foundations || [];
                        goalsData = data.goals || [];
                        userName = data.userName || '';
                        userJoinDate = data.joinDate || '';
                        console.log(`✅ Loaded from API: ${Object.keys(userEntriesByDay).length} entries, ${Object.keys(userReviewsByWeek).length} reviews, ${foundationEntries.length} foundation entries, ${goalsData.length} goals`);
                        if (foundationEntries.length > 0) {
                            console.log('📊 First foundation entry:', foundationEntries[0]);
                        }
                        if (goalsData.length > 0) {
                            console.log('📊 First goal:', goalsData[0]);
                        }
                    }
                }
            } catch (error) {
                console.log('Could not load user data, using empty journal:', error);
                userEntriesByDay = {};
                foundationEntries = [];
                goalsData = [];
            }
        ...
        // Get the latest foundation entry
        const foundation = foundationEntries && foundationEntries.length > 0 ? foundationEntries[0] : null;
        console.log('📊 Foundation data:', foundation ? 'Found' : 'Not found');
        if (foundation) {
            console.log('📊 Foundation keys:', Object.keys(foundation));
            console.log('📊 Foundation sample:', {
                what_drives_me: foundation.what_drives_me ? 'exists' : 'missing',
                books_that_shaped_me: foundation.books_that_shaped_me ? 'exists' : 'missing',
                my_vision: foundation.my_vision ? 'exists' : 'missing'
            });
        }
        console.log('📊 Goals data count:', goalsData?.length || 0);
        ...
        console.log('📦 Number of child nodes in tempContainer:', tempContainer.children.length);
        ...
        console.log(`📦 Appended ${appendedCount} child elements to main container`);
...
if (typeof window !== 'undefined') {
    ...
    console.log('✅ generateMasterJournal function registered globally');
    
    // Listen for postMessage from parent window (when loaded in iframe)
        window.addEventListener('message', function(event) {
            if (event.data && event.data.type === 'JOURNAL_ENTRIES') {
            console.log('📨 Received postMessage with journal data');
            userEntriesByDay = event.data.entriesByDay || {};
            userReviewsByWeek = event.data.reviewsByWeek || {};
            foundationEntries = event.data.foundations || [];
            goalsData = event.data.goals || [];
            userName = event.data.userName || '';
            userJoinDate = event.data.joinDate || '';
            console.log(`✅ Updated from postMessage: ${Object.keys(userEntriesByDay).length} entries, ${Object.keys(userReviewsByWeek).length} reviews, ${foundationEntries.length} foundation entries, ${goalsData.length} goals`);
            if (foundationEntries.length > 0) {
                console.log('📊 Foundation entry from postMessage:', Object.keys(foundationEntries[0]));
            }
            
            // Regenerate journal with new data
    ...
    // Also check for data set directly on window (fallback)
    if (window.userFoundationEntries) {
        foundationEntries = window.userFoundationEntries;
        console.log('✅ Found userFoundationEntries on window:', foundationEntries.length);
    }
    if (window.userGoals) {
        goalsData = window.userGoals;
        console.log('✅ Found userGoals on window:', goalsData.length);
    }
}

// Initialize when page loads
(function() {
    console.log('📄 Journal generator script loaded');
    console.log('Document ready state:', document.readyState);
    console.log('Journal content element exists:', !!document.getElementById('journal-content'));
    
    function initJournal() {
        console.log('📄 Initializing journal generation...');
        generateMasterJournal().then(() => {
...
// Helper function to format text into lines
function formatTextIntoLines(text, maxLines = 6) {
    if (!text) return Array(maxLines).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join('');
    
    // Convert to string and handle different separators
    const textStr = String(text);
    
    // Split by newlines, carriage returns, or semicolons (common separators)
    let lines = textStr
        .split(/\r?\n|;/)  // Split by newline, carriage return, or semicolon
        .map(l => l.trim())
        .filter(l => l.length > 0);  // Remove empty lines
    
    // If no lines found, try splitting by comma if the text is short
    if (lines.length === 1 && lines[0].length < 200) {
        const commaSplit = lines[0].split(',').map(l => l.trim()).filter(l => l.length > 0);
        if (commaSplit.length > 1) {
            lines = commaSplit;
        }
    }
    
    // If we have fewer lines than maxLines, try to wrap long lines
    if (lines.length < maxLines) {
        const wrappedLines = [];
        lines.forEach(line => {
            // If line is very long, try to split it at word boundaries
            if (line.length > 80) {
    ...
    const formatted = lines.map(line => `<div style="color: var(--white); padding: 0.05rem 0; font-size: 0.65rem; line-height: 1.3; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(line)}</div>`).join('');
    const emptyLines = Array(maxLines - lines.length).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.25rem 0; height: 18px;"></div>').join('');
    return formatted + emptyLines;
}
...
function formatListItems(text, maxItems = 5) {
    if (!text) return Array(maxItems).fill('').map((_, i) => 
        `<div style="display: flex; align-items: center; margin-bottom: 0.4rem;">
            <span style="color: var(--gold); font-size: 0.75rem; margin-right: 0.5rem;">${i + 1}.</span>
    ...
    let items = textStr
        .split(/\r?\n|;/)  // Split by newline, carriage return, or semicolon
        .map(l => l.trim())
        .filter(l => l.length > 0);  // Remove empty items
    
    // If no items found, try splitting by comma if the text is short
    if (items.length === 1 && items[0].length < 200) {
        const commaSplit = items[0].split(',').map(l => l.trim()).filter(l => l.length > 0);
        if (commaSplit.length > 1) {
            items = commaSplit;
        }
    }
    ...
    const formatted = items.map((item, i) => 
        `<div style="display: flex; align-items: center; margin-bottom: 0.25rem;">
            <span style="color: var(--gold); font-size: 0.7rem; margin-right: 0.4rem;">${i + 1}.</span>
            <div style="flex: 1; color: var(--white); padding: 0.05rem 0; font-size: 0.65rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(item)}</div>
        </div>`
    ).join('');
    const emptyItems = Array(maxItems - items.length).fill('').map((_, i) => 
        `<div style="display: flex; align-items: center; margin-bottom: 0.25rem;">
            <span style="color: var(--gold); font-size: 0.7rem; margin-right: 0.4rem;">${items.length + i + 1}.</span>
            <div style="flex: 1; border-bottom: 1px solid var(--gray-dark); height: 16px;"></div>
        </div>`
    ).join('');
    return formatted + emptyItems;
}

// Continue Chapter 1 - VISION
function generateChapter1VisionComplete() {
    // Get the latest foundation entry and goals
    // If multiple entries exist, combine them (take non-empty values from latest entry, fallback to earlier entries)
    let foundation = null;
    if (foundationEntries && foundationEntries.length > 0) {
        foundation = { ...foundationEntries[foundationEntries.length - 1] }; // Start with latest
        
        // Merge with earlier entries for any empty fields
        for (let i = foundationEntries.length - 2; i >= 0; i--) {
            const entry = foundationEntries[i];
            Object.keys(entry).forEach(key => {
                if (!foundation[key] || (typeof foundation[key] === 'string' && foundation[key].trim() === '')) {
                    foundation[key] = entry[key];
                } else if (typeof foundation[key] === 'string' && typeof entry[key] === 'string' && entry[key].trim() !== '') {
                    // If both have values, combine them with newline
                    foundation[key] = foundation[key] + '\n' + entry[key];
                }
            });
        }
        
        console.log('📊 Combined foundation entry:', {
            what_drives_me: foundation.what_drives_me?.substring(0, 100),
            what_im_done_with: foundation.what_im_done_with?.substring(0, 100),
            who_im_building_for: foundation.who_im_building_for?.substring(0, 100),
            books_that_shaped_me: foundation.books_that_shaped_me?.substring(0, 100),
            mentors_role_models: foundation.mentors_role_models?.substring(0, 100),
            core_principles: foundation.core_principles?.substring(0, 100),
            my_values: foundation.my_values?.substring(0, 100)
        });
    }
    
    // For legacy/vision/strategic goals, check if the goal has those fields populated
    // Also check if goals have timeline field set to match
    const legacyGoals = (goalsData || []).filter(g => {
        return (g.legacy_goals && g.legacy_goals.trim()) || 
               (g.timeline === 'legacy') || 
               (g.type === 'long' && !g.legacy_goals && !g.vision_goals && !g.strategic_goals);
    });
    const visionGoals = (goalsData || []).filter(g => {
        return (g.vision_goals && g.vision_goals.trim()) || 
               (g.timeline === 'vision') ||
               (g.type === 'long' && g.vision_goals);
    });
    const strategicGoals = (goalsData || []).filter(g => {
        return (g.strategic_goals && g.strategic_goals.trim()) || 
               (g.timeline === 'strategic') ||
               (g.type === 'long' && g.strategic_goals);
    });
    const tacticalGoals = (goalsData || []).filter(g => {
        return g.type === 'medium' || 
               g.timeline === 'tactical' ||
               (g.type === 'medium' && !g.legacy_goals && !g.vision_goals && !g.strategic_goals);
    });
    const quarterlyGoals = (goalsData || []).filter(g => {
        return g.type === 'short' || 
               g.timeline === 'quarterly' ||
               (g.type === 'short' && !g.legacy_goals && !g.vision_goals && !g.strategic_goals);
    });
    
    // Get vision data from goals - combine all goals that have vision data
    const visionGoalsWithData = (goalsData || []).filter(g => g.empire_vision || g.financial_freedom_number || g.legacy_impact);
    
    // Combine vision data from all goals (latest takes priority)
    let visionData = {
        empire_vision: '',
    ...
    // Start with the latest goal and merge with earlier ones
    visionGoalsWithData.reverse().forEach(goal => {
        if (goal.empire_vision && goal.empire_vision.trim() && !visionData.empire_vision) {
            visionData.empire_vision = goal.empire_vision;
        }
        if (goal.financial_freedom_number && goal.financial_freedom_number.trim() && !visionData.financial_freedom_number) {
            visionData.financial_freedom_number = goal.financial_freedom_number;
        }
        if (goal.legacy_impact && goal.legacy_impact.trim() && !visionData.legacy_impact) {
            visionData.legacy_impact = goal.legacy_impact;
        }
    });
    
    console.log('📊 Vision data found:', {
        goalsWithVisionData: visionGoalsWithData.length,
        empire_vision: visionData.empire_vision?.substring(0, 50) || 'empty',
        financial_freedom_number: visionData.financial_freedom_number?.substring(0, 50) || 'empty',
        legacy_impact: visionData.legacy_impact?.substring(0, 50) || 'empty'
    });
    
    console.log('📊 generateChapter1VisionComplete - Foundation:', foundation ? 'Found' : 'Not found');
    console.log('📊 generateChapter1VisionComplete - Goals count:', goalsData?.length || 0);
    console.log('📊 generateChapter1VisionComplete - Legacy goals:', legacyGoals.length);
    console.log('📊 generateChapter1VisionComplete - Vision goals:', visionGoals.length);
    console.log('📊 generateChapter1VisionComplete - Strategic goals:', strategicGoals.length);
    console.log('📊 generateChapter1VisionComplete - Tactical goals:', tacticalGoals.length);
    console.log('📊 generateChapter1VisionComplete - Quarterly goals:', quarterlyGoals.length);
    if (legacyGoals.length > 0) {
        console.log('📊 Sample legacy goal:', { title: legacyGoals[0].title, legacy_goals: legacyGoals[0].legacy_goals });
    }
    if (visionGoals.length > 0) {
        console.log('📊 Sample vision goal:', { title: visionGoals[0].title, vision_goals: visionGoals[0].vision_goals });
    }
        ...
        <div class="goal-timeline" style="display: grid; gap: 0.3rem;">
            ...
            <div class="timeline-section" style="background: var(--black-card); padding: 0.4rem; border-radius: 6px; border-left: 4px solid var(--gold);">
                ...
                <div class="goal-lines">
                    ${legacyGoals.length > 0 ? legacyGoals.slice(0, 6).map(g => {
                        // Use legacy_goals field if available, otherwise use title
                        const goalText = (g.legacy_goals && g.legacy_goals.trim()) ? g.legacy_goals : (g.title || '');
                        return `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(goalText)}</div>`;
                    }).join('') : Array(6).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.3rem 0; height: 18px;"></div>').join('')}
                </div>
            </div>
            ...
            <div class="timeline-section" style="background: var(--black-card); padding: 0.4rem; border-radius: 6px; border-left: 4px solid var(--gold-light);">
                ...
                <div class="goal-lines">
                    ${visionGoals.length > 0 ? visionGoals.slice(0, 6).map(g => {
                        // Use vision_goals field if available, otherwise use title
                        const goalText = (g.vision_goals && g.vision_goals.trim()) ? g.vision_goals : (g.title || '');
                        return `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(goalText)}</div>`;
                    }).join('') : Array(6).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.3rem 0; height: 18px;"></div>').join('')}
                </div>
            </div>
            ...
            <div class="timeline-section" style="background: var(--black-card); padding: 0.4rem; border-radius: 6px; border-left: 4px solid #FFD700;">
                ...
                <div class="goal-lines">
                    ${strategicGoals.length > 0 ? strategicGoals.slice(0, 6).map(g => {
                        // Use strategic_goals field if available, otherwise use title
                        const goalText = (g.strategic_goals && g.strategic_goals.trim()) ? g.strategic_goals : (g.title || '');
                        return `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(goalText)}</div>`;
                    }).join('') : Array(6).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.3rem 0; height: 18px;"></div>').join('')}
                </div>
            </div>
    ...
    <div class="page" id="page-tactical-execution" style="padding-top: 0.2rem;">
        ...
        <div class="tactical-goals" style="display: grid; gap: 0.4rem; margin-top: 0.3rem;">
            ...
            <div class="timeline-section" style="background: var(--black-card); padding: 0.4rem; border-radius: 6px; border-left: 4px solid #FFA500;">
                ...
                <div class="goal-lines">
                    ${tacticalGoals.length > 0 ? tacticalGoals.slice(0, 5).map(g => {
                        // Use description if available, otherwise title
                        const goalText = (g.description && g.description.trim()) ? g.description : (g.title || '');
                        return `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.7rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(goalText)}</div>`;
                    }).join('') : Array(5).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.4rem 0; height: 22px;"></div>').join('')}
                </div>
            </div>
            ...
            <div class="timeline-section" style="background: var(--black-card); padding: 0.4rem; border-radius: 6px; border-left: 4px solid #32CD32;">
                ...
                <div class="goal-categories" style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.3rem; margin-top: 0.2rem;">
                    <div>
                        ...
                        ${(quarterlyGoals.filter(g => g.category === 'career').slice(0, 3).map(g => {
                            const goalText = (g.description && g.description.trim()) ? g.description : (g.title || '');
                            return `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem; word-wrap: break-word; white-space: pre-wrap;">${escapeHtml(goalText)}</div>`;
                        }).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                    <div>
                        <h4 style="color: var(--gold-light); font-size: 0.65rem; margin-bottom: 0.1rem;">💪 HEALTH</h4>
                        ${(quarterlyGoals.filter(g => g.category === 'health').slice(0, 3).map(g => 
                            `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem;">${escapeHtml(g.title || '')}</div>`
                        ).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                    <div>
                        <h4 style="color: var(--gold-light); font-size: 0.65rem; margin-bottom: 0.1rem;">💰 FINANCIAL</h4>
                        ${(quarterlyGoals.filter(g => g.category === 'financial').slice(0, 3).map(g => 
                            `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem;">${escapeHtml(g.title || '')}</div>`
                        ).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                    <div>
                        <h4 style="color: var(--gold-light); font-size: 0.65rem; margin-bottom: 0.1rem;">❤️ RELATIONSHIPS</h4>
                        ${(quarterlyGoals.filter(g => g.category === 'relationships').slice(0, 3).map(g => 
                            `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem;">${escapeHtml(g.title || '')}</div>`
                        ).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                    <div>
                        <h4 style="color: var(--gold-light); font-size: 0.65rem; margin-bottom: 0.1rem;">🎓 LEARNING</h4>
                        ${(quarterlyGoals.filter(g => g.category === 'learning').slice(0, 3).map(g => 
                            `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem;">${escapeHtml(g.title || '')}</div>`
                        ).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                    <div>
                        <h4 style="color: var(--gold-light); font-size: 0.65rem; margin-bottom: 0.1rem;">🌟 GROWTH</h4>
                        ${(quarterlyGoals.filter(g => g.category === 'growth' || !g.category).slice(0, 3).map(g => 
                            `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.65rem;">${escapeHtml(g.title || '')}</div>`
                        ).join('') || Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join(''))}
                    </div>
                </div>
        ...
        <div class="commitment-section">
            ...
            <div class="commitment-pledge" style="background: var(--black-card); padding: 1rem; border-radius: 8px; border: 2px solid var(--gold); margin: 1rem 0;">
                ...
                <div class="pledge-text" style="color: var(--gray-light); line-height: 1.5; font-size: 0.8rem;">
                    <p style="margin-bottom: 0.5rem;">I, ${userName ? escapeHtml(userName) : (foundation?.accountability_partner ? escapeHtml(foundation.accountability_partner) : '_________________________')}, commit to:</p>
            ...
            <div class="accountability-partner" style="margin-top: 1rem;">
                ...
                ${foundation?.accountability_partner ? 
                    `<div style="color: var(--white); padding: 0.1rem 0; font-size: 0.7rem; margin: 0.3rem 0;">${escapeHtml(foundation.accountability_partner)}</div>` :
                    Array(4).fill('').map(() => '<div style="border-bottom: 1px solid var(--gold-dark); margin: 0.3rem 0; height: 18px;"></div>').join('')
                }
                ${!foundation?.accountability_partner ? '<p style="color: var(--gray); font-size: 0.7rem; margin-top: 0.2rem;">Name & Contact</p>' : ''}
            </div>
        </div>
    ...
    <div class="page" id="page-vision">
        ...
        <div class="vision-questions">
                ...
                <h3 style="color: var(--gold); margin-bottom: 0.3rem; font-size: 0.85rem;">🏰 My Empire Vision</h3>
                    ...
                    ${formatTextIntoLines(visionData.empire_vision || foundation?.my_vision || '', 6)}
                    </div>
                </div>
                ...
                <div class="writing-lines">
                    ${visionData.financial_freedom_number ? 
                        formatTextIntoLines(visionData.financial_freedom_number, 3) :
                        Array(3).fill('').map(() => '<div style="border-bottom: 1px solid var(--gray-dark); margin: 0.35rem 0; height: 22px;"></div>').join('')
                    }
                </div>
            </div>
            ...
            <div class="question-section mb-4">
                ...
                <div class="writing-lines">
                    ${formatTextIntoLines(visionData.legacy_impact || '', 5)}
                </div>
            </div>
            </div>
    ...
    <div class="page" id="page-why">
        ...
        <div class="why-questions">
            <div class="question-section mb-4">
                ...
                <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.what_drives_me || foundation?.my_why || '', 5)}
                </div>
            </div>
            ...
            <div class="question-section mb-4">
                ...
                <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.what_im_done_with || '', 5)}
                </div>
            </div>
                ...
                <h3 style="color: var(--gold); margin-bottom: 0.3rem; font-size: 0.85rem;">❤️ Who/What I'm Building For</h3>
                    ...
                    <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.who_im_building_for || '', 5)}
                    </div>
                </div>
        </div>
    ...
    <div class="page" id="page-influences">
        <h1 class="text-center gold-gradient mb-3" style="font-size: 1.6rem;">MY INFLUENCES</h1>
        <div class="gold-line" style="margin-bottom: 0.5rem;"></div>
        
        <div class="influences-section">
            <div class="question-section mb-3">
                <h3 style="color: var(--gold); margin-bottom: 0.2rem; font-size: 0.8rem;">📚 Books That Shaped Me</h3>
                <p style="color: var(--gray-light); margin-bottom: 0.2rem; font-style: italic; font-size: 0.65rem; line-height: 1.1;">
                    List the 5 books that most influenced your thinking:
                </p>
                <div class="book-list">
                    ${formatListItems(foundation?.books_that_shaped_me || '', 5)}
                    </div>
                </div>
            
            <div class="question-section mb-3">
                <h3 style="color: var(--gold); margin-bottom: 0.2rem; font-size: 0.8rem;">👥 Mentors & Role Models</h3>
                <p style="color: var(--gray-light); margin-bottom: 0.2rem; font-style: italic; font-size: 0.65rem; line-height: 1.1;">
                    Who do you study and learn from? (Living or historical)
                    </p>
                    <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.mentors_role_models || foundation?.influences || '', 3)}
                    </div>
                </div>
            
            <div class="question-section mb-3">
                <h3 style="color: var(--gold); margin-bottom: 0.2rem; font-size: 0.8rem;">💡 Core Principles</h3>
                <p style="color: var(--gray-light); margin-bottom: 0.2rem; font-style: italic; font-size: 0.65rem; line-height: 1.1;">
                    What are your 3 most important life principles?
                </p>
                <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.core_principles || '', 3)}
                </div>
            </div>
            
            <div class="question-section mb-3">
                <h3 style="color: var(--gold); margin-bottom: 0.2rem; font-size: 0.8rem;">⭐ My Values</h3>
                <p style="color: var(--gray-light); margin-bottom: 0.2rem; font-style: italic; font-size: 0.65rem; line-height: 1.1;">
                    What principles guide your decisions? What values do you live by?
                </p>
                <div class="writing-lines">
                    ${formatTextIntoLines(foundation?.my_values || '', 4)}
                </div>
            </div>
        </div>
        
        <div class="page-number">015</div>
        <div class="footer-wisdom" style="font-size: 0.7rem;">"You become what you think about most." — Earl Nightingale</div>
        </div>`;
}
...
function generateChapter3Do() {
    ...
    console.log('📊 generateChapter3Do - userEntriesByDay keys:', Object.keys(userEntriesByDay));
    console.log('📊 generateChapter3Do - userEntriesByDay sample:', userEntriesByDay[1] || userEntriesByDay['1'] || 'No entry for day 1');
        for (let day = 1; day <= 90; day++) {
        const quote = uniqueQuotes[day - 1];
            // Try both number and string key (API might return string keys)
            const entry = userEntriesByDay[day] || userEntriesByDay[String(day)] || null;
            if (entry && day <= 5) {
                console.log(`📊 Day ${day} entry found:`, {
                    hasGratitude: !!entry.gratitude,
                    hasPriority1: !!entry.priority_1,
                    hasReflection: !!entry.reflection
                });
            }
            
                // Add ID to first daily page
                const dailyPageId = day === 1 ? 'id="chapter-3-daily"' : '';
                html += generateDailyPage(day, quote, currentPageNumber, entry, dailyPageId);
            currentPageNumber++;
            
            // Add weekly review every 7th day (except day 90)
            if (day % 7 === 0 && day < 90) {
                const week = Math.ceil(day / 7);
                // Add ID to first weekly review
                const weeklyReviewId = week === 1 ? 'id="chapter-3-weekly"' : '';
                // Get review data for this week (try both number and string key)
                const review = userReviewsByWeek[week] || userReviewsByWeek[String(week)] || null;
                if (review && week === 1) {
                    console.log('📊 Week 1 review found:', { wins: review.wins?.substring(0, 50), lessons: review.lessons?.substring(0, 50), next_steps: review.next_steps?.substring(0, 50) });
                }
                html += generateWeeklyReview(week, currentPageNumber, weeklyReviewId, review);
                    currentPageNumber++;
            }
...
function generateDailyPage(day, quote, pageNumber, entry = null, pageId = '') {
    ...
    return `
    <!-- DAY ${day} -->
    <div class="page daily-page" ${pageId}>
...
function generateWeeklyReview(week, pageNumber, pageId = '', review = null) {
    // Get review data or use empty values
    const wins = review?.wins || '';
    const lessons = review?.lessons || '';
    const nextSteps = review?.next_steps || '';
    
    // Format wins into lines
    const winsLines = formatTextIntoLines(wins, 5);
    
    // Format lessons into lines
    const lessonsLines = formatTextIntoLines(lessons, 5);
    
    // Format next steps into lines
    const nextStepsLines = formatTextIntoLines(nextSteps, 5);
    
    return `
    <!-- WEEK ${week} REVIEW -->
    <div class="page weekly-review" ${pageId}>
    ...
    function attemptQRGeneration(retries = 5) {
        ...
        if (typeof QRCode === 'undefined') {
            if (retries > 0) {
                ...
                // Use fallback for both chapters
                if (qrChapter1) {
